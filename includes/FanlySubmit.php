...
//remove plugin 
add_filter( 'plugin_action_links', 'fanly_submit_remove_plugin_link', 10, 4 );
function fanly_submit_remove_plugin_link( $actions, $plugin_file, $plugin_data, $context ){
	if(isset($actions['edit'])){
		switch($plugin_file){
			case 'Fanly-Submit/FanlySubmit.php':unset( $actions['edit'] );
			break;
		}
	}
	return $actions;
...
function fanly_submit_menu() {
	add_submenu_page('options-general.php','百度熊掌号/原创保护', 'Fanly Submit', 'manage_options', 'Fanly_Submit','fanly_submit_options', '');
}
//设置页面
function fanly_submit_options() {
	...
	if( $data = @FANLY_SUBMIT_CHECK() ){
		...
		if($ver > $plugin_data['Version']){
			echo '<div class="notice notice-warning"><p>Fanly Submit 插件已经有新版本啦！ <a target="_blank" href="'.$url.'">立即查看</a></p></div>';
		}
	}
	//保存数据
	...
	1.熊掌号 APPID/Token 通过 百度搜索资源平台-熊掌号-API提交-推送接口 > 接口调用地址获取；<br>
	2.其它相关问题至泪雪建站 <a target="_blank" href="https://zhan.leiue.com/fanly-submit.html">Fanly Submit 插件</a>页面查看使用说明和留言反馈。<br>
	</p>';
	echo '</div>';
}
?>